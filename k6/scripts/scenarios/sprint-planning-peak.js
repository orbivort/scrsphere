import http from 'k6/http';
import { check, sleep } from 'k6';
import { Rate, Trend, Counter } from 'k6/metrics';
import { config } from '../lib/config.js';
import {
  authenticate,
  logout,
  randomFloat,
  randomItem,
  generateTeamUserId,
  generatePbiData,
  generateTaskData,
  getAuthHeaders,
  getUserTeams,
  getActiveSprint,
} from '../lib/helpers.js';
import { recordResponseTime, recordError, recordOperation } from '../lib/metrics.js';

const errorRate = new Rate('errors');
const responseTime = new Trend('response_time');
const requestCount = new Counter('requests');
const pbiCreated = new Counter('pbi_created');
const taskCreated = new Counter('task_created');

export const options = {
  stages: [
    { duration: '30s', target: 5 },
    { duration: '1m', target: 15 },
    { duration: '10m', target: 15 },
    { duration: '2m', target: 20 },
    { duration: '2m', target: 15 },
    { duration: '30s', target: 0 },
  ],
  thresholds: {
    http_req_duration: ['p(95)<500', 'p(99)<800'],
    http_req_failed: ['rate<0.01'],
    errors: ['rate<0.01'],
  },
};

export default function sprintPlanningPeak() {
  const { teamNum, email } = generateTeamUserId(__VU, 1, 15);

  const { token, csrfToken, csrfCookie, jar } = authenticate(email, 'LoadTest123!');

  if (!token) {
    errorRate.add(1);
    recordError('auth', true);
    return;
  }

  recordOperation('login');

  const headers = getAuthHeaders(token, csrfToken);

  const teams = getUserTeams(token, csrfToken, jar);

  if (!teams || teams.length === 0) {
    recordError('api', true);
    logout(token, csrfToken, csrfCookie);
    return;
  }

  const team = teams[0];
  const teamId = team.id;

  const backlogRes = http.get(`${config.baseUrl}/api/v1/product-backlog?teamId=${teamId}`, {
    headers,
    jar,
    tags: { operation: 'getBacklog' },
  });

  check(backlogRes, {
    'backlog loaded': (r) => r.status === 200,
    'backlog response time OK': (r) => r.timings.duration < 400,
  });

  recordResponseTime('backlog', backlogRes.timings.duration);
  responseTime.add(backlogRes.timings.duration);
  requestCount.add(1);

  sleep(randomFloat(5, 15));

  const pbiData = generatePbiData(teamId, __VU, __ITER);
  const pbiRes = http.post(`${config.baseUrl}/api/v1/product-backlog`, JSON.stringify(pbiData), {
    headers,
    jar,
    tags: { operation: 'createPbi' },
  });

  check(pbiRes, {
    'PBI created': (r) => r.status === 201 || r.status === 200,
    'PBI response time OK': (r) => r.timings.duration < 500,
  });

  recordResponseTime('backlog', pbiRes.timings.duration);
  responseTime.add(pbiRes.timings.duration);
  requestCount.add(1);

  if (pbiRes.status === 201 || pbiRes.status === 200) {
    pbiCreated.add(1);
    recordOperation('pbiCreate');
  }

  sleep(randomFloat(10, 30));

  const sprint = getActiveSprint(teamId, token, csrfToken, jar);

  if (sprint && sprint.id) {
    const taskData = generateTaskData(sprint.id, null, __VU, __ITER);
    const taskRes = http.post(
      `${config.baseUrl}/api/v1/sprints/${sprint.id}/tasks`,
      JSON.stringify(taskData),
      { headers, jar, tags: { operation: 'createTask' } }
    );

    check(taskRes, {
      'task created': (r) => r.status === 201 || r.status === 200,
      'task response time OK': (r) => r.timings.duration < 500,
    });

    recordResponseTime('task', taskRes.timings.duration);
    responseTime.add(taskRes.timings.duration);
    requestCount.add(1);

    if (taskRes.status === 201 || taskRes.status === 200) {
      taskCreated.add(1);
    }

    sleep(randomFloat(5, 15));

    const boardRes = http.get(`${config.baseUrl}/api/v1/sprints/${sprint.id}/tasks`, {
      headers,
      jar,
      tags: { operation: 'getBoard' },
    });

    check(boardRes, {
      'board loaded': (r) => r.status === 200,
    });

    requestCount.add(1);

    sleep(randomFloat(3, 8));
  }

  logout(token, csrfToken, csrfCookie);
  recordOperation('logout');
}

export function handleSummary(data) {
  const p95 = data.metrics.http_req_duration?.values?.['p(95)'] || 0;
  const errorRateValue = (data.metrics.http_req_failed?.values?.rate || 0) * 100;

  return {
    stdout: `
========================================
Sprint Planning Peak Test Summary
========================================
Total Requests: ${data.metrics.http_reqs?.values?.count || 0}
Request Rate: ${(data.metrics.http_reqs?.values?.rate || 0).toFixed(2)} req/s
PBIs Created: ${data.metrics.pbi_created?.values?.count || 0}
Tasks Created: ${data.metrics.task_created?.values?.count || 0}

Response Times:
  - P50: ${(data.metrics.http_req_duration?.values?.['p(50)'] || 0).toFixed(2)}ms
  - P90: ${(data.metrics.http_req_duration?.values?.['p(90)'] || 0).toFixed(2)}ms
  - P95: ${p95.toFixed(2)}ms
  - P99: ${(data.metrics.http_req_duration?.values?.['p(99)'] || 0).toFixed(2)}ms

Error Rate: ${errorRateValue.toFixed(2)}%

Thresholds:
  - P95 < 500ms: ${p95 < 500 ? 'PASS' : 'FAIL'}
  - Error Rate < 1%: ${errorRateValue < 1 ? 'PASS' : 'FAIL'}

Overall Status: ${p95 < 500 && errorRateValue < 1 ? 'PASS' : 'FAIL'}
========================================
`,
    'k6/results/sprint-planning-peak-summary.json': JSON.stringify(data, null, 2),
  };
}
