...
describe('TaskEditModal', () => {
  ...
  describe('Rendering', () => {
    it('should render modal when isOpen', () => {
      renderWithProviders(<TaskEditModal {...defaultProps} />);
  ...
  describe('User Interactions', () => {
    it('should call onFormDataChange when title changes', () => {
      ...
      renderWithProviders(<TaskEditModal {...defaultProps} onFormDataChange={onFormDataChange} />);
    ...
    it('should call onSubmit when form is submitted', async () => {
      ...
      const user = userEvent.setup();
      renderWithProviders(<TaskEditModal {...defaultProps} onSubmit={onSubmit} />);
    ...
    it('should call onClose when Cancel button is clicked', async () => {
      const onClose = vi.fn();
      const user = userEvent.setup();
      renderWithProviders(<TaskEditModal {...defaultProps} onClose={onClose} />);
    ...
    it('should call onBackToDetails when Back to Details button is clicked', async () => {
      ...
      const user = userEvent.setup();
      renderWithProviders(<TaskEditModal {...defaultProps} onBackToDetails={onBackToDetails} />);
  ...
  describe('Form Validation', () => {
    it('should display error message when title has error', () => {
      renderWithProviders(
        <TaskEditModal {...defaultProps} formErrors={{ title: 'Title is required' }} />
      );
    ...
    it('should display error message when estimated hours has error', () => {
      renderWithProviders(
        <TaskEditModal {...defaultProps} formErrors={{ estimatedHours: 'Invalid hours' }} />
      );
    ...
    it('should display error message when remaining hours has error', () => {
      renderWithProviders(
        <TaskEditModal {...defaultProps} formErrors={{ remainingHours: 'Invalid hours' }} />
      );
      expect(screen.getByText('Invalid hours')).toBeInTheDocument();
    });
  ...
  describe('Workflow Error', () => {
    it('should display workflow error when present', () => {
      renderWithProviders(
        <TaskEditModal {...defaultProps} workflowError="Failed to update task" />
      );
    ...
    it('should not display workflow error banner when null', () => {
      renderWithProviders(<TaskEditModal {...defaultProps} workflowError={null} />);
      expect(screen.queryByRole('alert')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Loading State', () => {
    it('should disable form buttons when isUpdating is true', () => {
      renderWithProviders(<TaskEditModal {...defaultProps} isUpdating={true} />);
    ...
    it('should disable close via Escape when isUpdating', () => {
      const onClose = vi.fn();
      renderWithProviders(<TaskEditModal {...defaultProps} isUpdating={true} onClose={onClose} />);
  ...
  describe('Accessibility', () => {
    ...
    it('should have aria-invalid on inputs with errors', () => {
      renderWithProviders(<TaskEditModal {...defaultProps} formErrors={{ title: 'Error' }} />);
      const input = screen.getByLabelText(/Title/);
  ...
  describe('Overlay and Modal Behavior', () => {
    it('should call handleCloseAttempt when overlay is clicked', () => {
      ...
      const { container } = renderWithProviders(
        <TaskEditModal {...defaultProps} onClose={onClose} />
      );
  ...
  describe('Edge Cases', () => {
    it('should handle empty sprint items list', () => {
      renderWithProviders(<TaskEditModal {...defaultProps} sprintItems={[]} />);
      const select = screen.getByLabelText(/Parent Backlog Item/) as HTMLSelectElement;
      expect(select.options.length).toBe(1);
    });

    it('should handle empty team members list', () => {
      renderWithProviders(<TaskEditModal {...defaultProps} teamMembers={[]} />);
    ...
    it('should handle undefined user on team member gracefully', () => {
      ...
      renderWithProviders(<TaskEditModal {...defaultProps} teamMembers={teamMembers} />);
      const select = screen.getByLabelText(/Assignee/) as HTMLSelectElement;
  ...
  describe('Unsaved Changes Modal', () => {
    it('should show unsaved changes modal when hasUnsavedChanges is true and closing', () => {
      ...
      const { container } = renderWithProviders(<TaskEditModal {...defaultProps} />);
