...
describe('CharacterCounter', () => {
  ...
  it('should render current and max count', () => {
    renderWithProviders(<CharacterCounter current={50} max={100} />);
  ...
  it('should show neutral state when empty', () => {
    renderWithProviders(<CharacterCounter current={0} max={100} />);
  ...
  it('should show warning state when under minimum', () => {
    renderWithProviders(<CharacterCounter current={2} min={5} max={100} showMin />);
  ...
  it('should show error state when over limit', () => {
    renderWithProviders(<CharacterCounter current={150} max={100} />);
  ...
  it('should not show min indicator when showMin is false', () => {
    renderWithProviders(<CharacterCounter current={2} min={5} max={100} showMin={false} />);

    expect(screen.queryByText(/min/)).not.toBeInTheDocument();
  });

  it('should not show min indicator when current is above minimum', () => {
    renderWithProviders(<CharacterCounter current={10} min={5} max={100} showMin />);
